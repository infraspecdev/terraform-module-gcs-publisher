...
import { run, getValidatedInputs, validateInputs, createTempCredentialsFile, processModuleUpload, createZipArchive } from '../src/main';
...
jest.mock('../src/utils/file-utils', () => ({
  calculateFileHash: jest.fn().mockImplementation(() => Promise.resolve('mocked-file-hash'))
}));
...
describe('Main Action', () => {
  ...
  describe('Input Validation', () => {
    ...
    test('validateInputs validates input parameters', () => {
      ...
      const invalidBucketOption = {...validOptions, bucketName: 'INVALID-BUCKET'};
      expect(() => validateInputs(invalidBucketOption)).toThrow(/Invalid bucket name/i);
      
      // Should throw for invalid module name
      const invalidModuleOption = {...validOptions, moduleName: 'invalid.module'};
      expect(() => validateInputs(invalidModuleOption)).toThrow(/Invalid module name/i);
      
      // Should throw for invalid version
      const invalidVersionOption = {...validOptions, moduleVersion: 'not-a-version'};
      expect(() => validateInputs(invalidVersionOption)).toThrow(/Invalid module version/i);
    });
  });
  ...
  describe('Credentials Handling', () => {
    ...
    test('createTempCredentialsFile throws error with invalid JSON', () => {
      ...
      expect(() => createTempCredentialsFile(invalidCredentials)).toThrow(/Invalid.*JSON/i);
  ...
  describe('Zip Archive Creation', () => {
    ...
    test('creates a zip archive successfully', async () => {
      ...
      try {
        ...
        expect(exec.exec).toHaveBeenCalledWith('zip', 
  ...
  describe('Module Upload Processing', () => {
    ...
    const mockUploadToGCS = jest.fn().mockResolvedValue('https://storage.googleapis.com/test-bucket/modules/test-module/test-module-1.0.0.zip');
    ...
    test('successfully processes module upload with cleanup', async () => {
      ...
      expect(expectedResult).toBe('gs://test-bucket/modules/test-module/test-module-1.0.0.zip');
    });
  ...
  describe('Direct Module Upload Test', () => {
    ...
    beforeEach(() => {
      ...
      (calculateFileHash as jest.Mock).mockImplementation(() => Promise.resolve(mockFileHash));
    });
    ...
    test('processModuleUpload handles module upload with cleanup', async () => {
      ...
      const result = await processModuleUpload(options, mockGcsService as GCSService, zipFilePath);
      
      // Verify result matches expected GCS path
      expect(result).toBe('gs://test-bucket/modules/test-module/test-module-1.0.0.zip');
    ...
    test('processModuleUpload skips cleanup when deleteOldVersions is false', async () => {
      ...
      // Call the function being tested
      // Type assertion with our GCSServiceMock type is sufficient for the test
      await processModuleUpload(options, mockGcsService as GCSService, zipFilePath);
