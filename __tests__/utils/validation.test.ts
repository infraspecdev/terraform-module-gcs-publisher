...
describe('Validation Utilities', () => {
  describe('validateBucketName', () => {
    test('accepts valid bucket names', () => {
      ...
      expect(validateBucketName('another-valid-name')).toBe(
        'another-valid-name'
      )
      expect(validateBucketName('valid.bucket.with.periods')).toBe(
        'valid.bucket.with.periods'
      )
      expect(validateBucketName('valid_bucket_with_underscores')).toBe(
        'valid_bucket_with_underscores'
      )
    })
  ...
  describe('validateModuleName', () => {
    test('accepts valid module names', () => {
      ...
      expect(validateModuleName('ValidModuleName123')).toBe(
        'ValidModuleName123'
      )
    })
