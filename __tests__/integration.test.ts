...
const createTempTerraformModule = (): string => {
  const tempDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'terraform-module-test-')
  )
  ...
  fs.writeFileSync(
    path.join(tempDir, 'variables.tf'),
...
describe('Terraform Module GCS Publisher Integration', () => {
  ...
  test('uploads Terraform module and cleans up old versions', async () => {
    ...
    const mockRun = async (): Promise<boolean> => {
      ...
      core.setOutput(
        'module-url',
        `https://storage.googleapis.com/${bucketName}/modules/${moduleName}/${moduleName}-${moduleVersion}.zip`
      )
    ...
    expect(core.getInput).toHaveBeenCalledWith('gcs-bucket', {
      required: true
    })
    expect(core.getInput).toHaveBeenCalledWith('module-name', {
      required: true
    })
    expect(core.getInput).toHaveBeenCalledWith('module-version', {
      required: true
    })
    expect(core.getInput).toHaveBeenCalledWith('module-path', {
      required: true
    })
    expect(core.getInput).toHaveBeenCalledWith('google-credentials', {
      required: true
    })
    ...
    expect(core.setOutput).toHaveBeenCalledWith(
      'module-url',
      expect.stringContaining('test-terraform-modules')
    )
